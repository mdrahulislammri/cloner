...
if (($settings['chat_messenger_enabled'] ?? '1') === '1' && trim((string)($settings['messenger_url'] ?? '')) !== '') {
    $channels[] = ['label' => 'Messenger', 'icon' => 'fa-brands fa-facebook-messenger', 'url' => (string)$settings['messenger_url']];
}

if (($settings['chat_telegram_enabled'] ?? '1') === '1' && trim((string)($settings['telegram_url'] ?? '')) !== '') {
    $channels[] = ['label' => 'Telegram', 'icon' => 'fa-brands fa-telegram', 'url' => (string)$settings['telegram_url']];
}

if (($settings['chat_whatsapp_enabled'] ?? '1') === '1') {
    $channels[] = ['label' => 'WhatsApp', 'icon' => 'fa-brands fa-whatsapp', 'url' => 'https://wa.me/' . $whatsappNumber];
}
...
function serviceIconClass(string $raw): string
{
    $value = trim($raw);
    $emojiMap = [
        '📘' => 'fa-brands fa-facebook-f',
        '📸' => 'fa-brands fa-instagram',
        '▶️' => 'fa-brands fa-youtube',
        '🟢' => 'fa-solid fa-bolt',
    ];

    if (isset($emojiMap[$value])) {
        return $emojiMap[$value];
    }

    $sanitized = preg_replace('/[^a-z0-9\-\s]/i', '', $value) ?? '';
    $sanitized = trim($sanitized);
    return $sanitized !== '' ? $sanitized : 'fa-solid fa-bolt';
}

function chatIconClass(string $raw): string
{
    $value = trim($raw);
    $sanitized = preg_replace('/[^a-z0-9\-\s]/i', '', $value) ?? '';
    $sanitized = trim($sanitized);
    return $sanitized !== '' ? $sanitized : 'fa-solid fa-comments';
}

function chatIconTone(string $label): string
{
    $tones = [
        'messenger' => 'chat-link-icon--messenger',
        'telegram' => 'chat-link-icon--telegram',
        'whatsapp' => 'chat-link-icon--whatsapp',
        'call' => 'chat-link-icon--call',
    ];

    $key = strtolower(trim($label));
    return $tones[$key] ?? 'chat-link-icon--default';
}

$callNumber = trim((string)($settings['call_number'] ?? ($settings['phone'] ?? '')));
if (($settings['chat_call_enabled'] ?? '1') === '1' && $callNumber !== '') {
    $channels[] = ['label' => 'Call', 'icon' => 'fa-solid fa-phone-volume', 'url' => 'tel:' . preg_replace('/[^\d+]/', '', $callNumber)];
}
?>
<!doctype html>
<html lang="bn">
<head>
  ...
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.2/css/all.min.css" integrity="sha512-SnH5WK+bZxgPHs44uWix+LLJAJ9/2PkPKZ5QiAj6Ta86w+fsb2TkR4j8R2YV8mWQhWQ9f2MYYbZ6+4Q6w5P6Kw==" crossorigin="anonymous" referrerpolicy="no-referrer" />
          ...
          <?php foreach ($services as $service): ?>
            <article class="service-card group">
              <div class="service-card-top">
                <div class="service-icon"><i class="<?= htmlspecialchars(serviceIconClass((string)$service['icon'])) ?>" aria-hidden="true"></i></div>
    ...
    <div class="chat-float" data-chat-widget>
      <div class="chat-menu hidden" data-chat-menu>
        <div class="chat-menu-head">
          <span class="chat-head-icon" aria-hidden="true"><i class="fa-solid fa-headset"></i></span>
          <div>
        ...
        <div class="chat-links">
          <?php foreach ($channels as $channel): ?>
            <a href="<?= htmlspecialchars($channel['url']) ?>" class="chat-link" target="_blank" rel="noopener">
              <span class="chat-link-icon <?= htmlspecialchars(chatIconTone((string)$channel['label'])) ?>" aria-hidden="true"><i class="<?= htmlspecialchars(chatIconClass((string)$channel['icon'])) ?>"></i></span>
              <span class="chat-link-text"><?= htmlspecialchars($channel['label']) ?></span>
              <span class="chat-link-arrow" aria-hidden="true"><i class="fa-solid fa-angle-right"></i></span>
            </a>
          <?php endforeach; ?>
        </div>
      </div>
      <button type="button" class="wa-float" data-chat-toggle aria-expanded="false" aria-label="Open contact options">
        ...
        <span class="wa-icon" data-chat-icon><i class="fa-solid fa-comments"></i></span>
      </button>
    </div>
  <?php endif; ?>
