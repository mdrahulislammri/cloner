...
<div class="bg-white p-4 rounded shadow"><?php foreach($services as $row): ?><div class="border-b py-2"><?= htmlspecialchars($row['icon']) ?> <strong><?= htmlspecialchars($row['title']) ?></strong> - <?= htmlspecialchars($row['description']) ?> <button type="button" class="text-red-600 text-sm" data-confirm-action data-confirm-title="Delete service" data-confirm-message="এই সার্ভিসটি মুছে ফেলতে চান?" data-confirm-yes="Yes, Delete" data-confirm-no="No" data-confirm-form-id="delete-service-<?= (int)$row['id'] ?>">Delete</button><form id="delete-service-<?= (int)$row['id'] ?>" method="post" class="hidden"><input type="hidden" name="csrf" value="<?= htmlspecialchars(csrfToken()) ?>"><input type="hidden" name="delete_id" value="<?= (int)$row['id'] ?>"></form></div><?php endforeach; ?></div>
