...
<div class="bg-white p-4 rounded shadow"><?php foreach($items as $row): ?><div class="border-b py-2"><strong><?= htmlspecialchars($row['client_name']) ?></strong> (<?= (int)$row['rating'] ?>/5): <?= htmlspecialchars($row['content']) ?> <button type="button" class="text-red-600 text-sm" data-confirm-action data-confirm-title="Delete testimonial" data-confirm-message="এই রিভিউটি মুছে ফেলতে চান?" data-confirm-yes="Yes, Delete" data-confirm-no="No" data-confirm-form-id="delete-testimonial-<?= (int)$row['id'] ?>">Delete</button><form id="delete-testimonial-<?= (int)$row['id'] ?>" method="post" class="hidden"><input type="hidden" name="csrf" value="<?= htmlspecialchars(csrfToken()) ?>"><input type="hidden" name="delete_id" value="<?= (int)$row['id'] ?>"></form></div><?php endforeach; ?></div>
