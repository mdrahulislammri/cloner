...
adminLayoutStart('Visitor Monitoring', 'monitoring');
?>
...
<div class="grid lg:grid-cols-3 gap-4">
  <div class="bg-white rounded-xl p-4 shadow border border-emerald-100 lg:col-span-2">
    <h3 class="font-bold text-slate-800 mb-3">Last 7 Days Visit Trend</h3>
    <canvas id="visitChart" height="120"></canvas>
  </div>
  <div class="bg-white rounded-xl p-4 shadow border border-emerald-100">
    <h3 class="font-bold text-slate-800 mb-3">Device Split</h3>
    <canvas id="deviceChart" height="180"></canvas>
  </div>
</div>

<div class="bg-white rounded-xl p-4 shadow border border-emerald-100">
  <h3 class="font-bold text-slate-800 mb-3">Recent Visitors</h3>
  <div class="overflow-x-auto">
    <table class="w-full text-sm min-w-[640px]">
      <thead>
        <tr class="text-left text-slate-500 border-b">
          <th class="py-2 pr-3">Time</th><th class="py-2 pr-3">Device</th><th class="py-2 pr-3">Referrer</th><th class="py-2">Page</th>
        </tr>
      </thead>
      <tbody>
      <?php if ($summary['recent'] === []): ?>
        <tr><td colspan="4" class="py-6 text-center text-slate-500">No visit data yet.</td></tr>
      <?php else: foreach ($summary['recent'] as $row): ?>
...
<script>
  ...
  const deviceValues = [<?= (int)($summary['device_counts']['Desktop'] ?? 0) ?>, <?= (int)($summary['device_counts']['Mobile'] ?? 0) ?>, <?= (int)($summary['device_counts']['Tablet'] ?? 0) ?>];

  new Chart(document.getElementById('visitChart'), {type:'line', data:{labels:visitLabels, datasets:[{data:visitValues, borderColor:'#059669', backgroundColor:'rgba(16,185,129,.12)', fill:true, tension:0.35, pointRadius:4}]}, options:{responsive:true, maintainAspectRatio:false, plugins:{legend:{display:false}}}});
  new Chart(document.getElementById('deviceChart'), {type:'doughnut', data:{labels:['Desktop','Mobile','Tablet'], datasets:[{data:deviceValues, backgroundColor:['#065f46','#10b981','#6ee7b7']}]}, options:{responsive:true, maintainAspectRatio:false, plugins:{legend:{position:'bottom'}}}});
</script>
<?php adminLayoutEnd(); ?>
